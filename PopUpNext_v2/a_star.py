...
################################################################################
class Node():
# """______________________ Kaelen stuff___________________________"""
...
def trafficBlock(maze, start1, end):
    ...
    path_arr = [astar(param.tmaze, start1, path[int(sur_ind[0]-1)][::-1])]

    if sur_ind.size > 2:

        for i in range(1, len(sur_ind)):
            path_arr.append(astar(param.tmaze), path[sur_ind[i-1]+1][::-1], path[sur_ind[i]-1][::-1])

    path_arr.append(astar(param.tmaze, path[int(sur_ind[-1]+1)][::-1], end))

...
def create_traffic(tmaze):
    ...
    for x in range(len(tmaze)):
        for y in range(len(tmaze[0])):
            # make into functions
            if tmaze[x][y] != 0 and tmaze[x][y] != 4 and not x % 2 == 0: #Makes a rectangle between buildings in the x axis
            ...
            if tmaze[x][y] != 0 and tmaze[x][y] != 4 and not y % 2 == 0: #Makes a rectangle between buildings in the y axis
